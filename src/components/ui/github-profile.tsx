...
export default function GitHubProfileViewer() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-blue-50 via-gray-100 to-blue-50 py-8">
  <Card className="w-full max-w-3xl p-6 space-y-4 shadow-lg rounded-lg bg-white">
    <CardHeader className="text-center">
      <CardTitle className="text-3xl font-bold mb-2 text-teal-700">
        GitHub Profile Viewer
      </CardTitle>
      <CardDescription className="text-gray-600">
        Search for a GitHub username and view their profile and repositories, search like: taha12-ok
      </CardDescription>
    </CardHeader>
    ...
    <form onSubmit={handleSubmit} className="mb-8 px-6">
      <div className="flex items-center gap-4">
        <Input
          ...
          className="flex-1 border rounded-md p-2 focus:ring-teal-500 focus:border-teal-500"
        />
        <Button
          type="submit"
          disabled={loading}
    ...
    {userProfile && (
      <div className="grid gap-8 px-6">
        ...
        <div className="grid md:grid-cols-[120px_1fr] gap-6">
          ...
          <div className="grid gap-2">
            <div className="flex items-center gap-2">
              <h2 className="text-2xl font-bold text-teal-700">{userProfile.login}</h2>
              <Link href={userProfile.html_url} target="_blank" className="text-teal-700 hover:text-teal-600">
            ...
            <div className="flex items-center gap-4 text-sm">
              ...
              <div className="flex items-center gap-1">
                <LocateIcon className="w-4 h-4 text-teal-600" />
        ...
        <div className="grid gap-6">
          <h3 className="text-xl font-bold text-teal-700">Repositories</h3>
          <div className="grid sm:grid-cols-2 lg:grid-cols-2 gap-6">
            {userRepos.map((repo) => (
              <Card key={repo.id} className="shadow-md rounded-lg bg-white border hover:shadow-lg transition-shadow duration-200">
                <CardHeader>
                  <div className="flex items-center gap-2">
                    <RecycleIcon className="w-6 h-6 text-teal-600" />
                    <CardTitle>
                      <Link href={repo.html_url} target="_blank" className="hover:text-teal-700">
                        {repo.name}
                      </Link>
                    </CardTitle>
                  </div>
                ...
                <CardFooter className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    <StarIcon className="w-4 h-4 text-teal-600" />
                    <span>{repo.stargazers_count}</span>
                    <ForkliftIcon className="w-4 h-4 text-teal-600" />
                    <span>{repo.forks_count}</span>
                  </div>
                  <Link href={repo.html_url} target="_blank" className="text-teal-700 hover:underline">
                    View on GitHub
                  </Link>
                </CardFooter>
              </Card>
